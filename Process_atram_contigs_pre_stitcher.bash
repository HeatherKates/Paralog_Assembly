...
#add the sample and locus name to the fasta header
name=`echo $x|cut -c 1-9`;sed "s/>/>$name /g" $x.shortremoved > $x.named
#Make sequence headers unique
awk '{$1=$1"."++a[$1]}1' $x.named > $x.uniq
#Change to two digit notation #added
sed -i 's/\.\([0-9]\{1\}\) /.0\1 /g'  $x.uniq
#back on two lines
sed -i -r 's/(.*) /\1\n/' $x.uniq
#Move every two lines to a new file and name file based on first line
grep ">" $x.uniq > $x.uniqIDs
while read file; do grep -w -A1 `echo $file|cut -c 1-13` ${x}.uniq > `echo $file|cut -c 2-13`_contig_Locus_$i.fasta; done < $x.uniqIDs #edited from -c 1/2-12 to 1/2-13
#cleanup
rm $x.uniq
rm $x.uniqIDs
